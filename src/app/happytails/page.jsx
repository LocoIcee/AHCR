...
const HappyTailsPage = () => {
  ...
  return (
    <div>
      {/* Banner Section */}
      <section className=" pt-24 relative w-full bg-gradient-to-r from-tealGrey to-primary py-2 px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-24">
          ...
          <img
            src="images/siteDogs/ThankYou.png"
            alt="Thank You"
          ...
          {happyTails.map((dog) => (
            <div key={dog.id} className="bg-white shadow-md rounded-md overflow-hidden relative">
              <div className="relative">
                {(() => {
                  const firstImage = Array.isArray(dog.images) ? dog.images[0] : dog.images;
                  if (!firstImage) return null;
                  return /\.(mp4|webm|ogg)$/i.test(firstImage) ? (
                    <video
                      src={firstImage}
                      controls
                      className="w-full h-64 object-cover"
                      preload="metadata"
                      poster={firstImage + '#t=1'}
                    />
                  ) : (
                    <img
                      src={firstImage}
